...
def get_lyrics(playlist_id):
    playlist_json=get_playlist(playlist_id)
    lyrics_json={}
    for index, value in playlist_json.items():
        track_lyrics_dict={}
        artist = value['artist']
        track =  value['song']
...
def get_playlist_event():
    ...
    playlist_event = {'event_type': 'playlist',
                            'playlist_json': playlist_json}
    log_to_kafka('events', playlist_event)
...
def get_lyrics_event():
    ...
    lyrics_json = get_lyrics(playlist_id)

    lyrics_event = {'event_type': 'lyrics',
                    'lyrics_json':lyrics_json}
    log_to_kafka('events', lyrics_event)
